...
export default function BasicTableFour() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        const formattedData: Order[] = data.map((user: any) => ({
          id: user.id,
          name: user.name,
          email: user.email,
          role: user.role,
  ...
  const exportPDF = () => {
  const doc = new jsPDF();
  const tableColumn = ["S.No", "User", "Email", "Role", "Action"];
  ...
  const copyToClipboard = () => {
    ...
    filteredData.forEach((order, index) => {
      copyText += `${index + 1}\t${order.name}\t${order.email}\n`;
    });
                ...
                <TableRow key={order.id}>
                  ...
                  <TableCell className="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400">
                    <div className="flex gap-3 items-center">
                      <button onClick={() => navigate(`/viewuser/${order.id}`)} title="View">
                        <Eye className="w-5 h-5 text-blue-500 hover:text-blue-700" />
                      </button>
                      <button onClick={() => navigate(`/edituser/${order.id}`)} title="Edit">
