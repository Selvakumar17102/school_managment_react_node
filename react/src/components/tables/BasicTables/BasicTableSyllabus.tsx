...
export default function BasicTableSyllabus() {
    ...
    const fetchsyllabus = async () => {
      try {
        ...
        const formattedData: Order[] = data.map((syllabus: any) => ({
          ...
          files: syllabus.files ? JSON.parse(syllabus.files) : [],
  ...
  const exportPDF = () => {
    const doc = new jsPDF();
    const tableColumn = ["S.No", "Class Name", "Title", "Description", "File"];
    const tableRows: (string | number)[][] = [];

    filteredData.forEach((order, index) => {
      const fileNames = Array.isArray(order.files)
        ? order.files.map((file: File) => file.name).join(", ")
  ...
  const copyToClipboard = () => {
    let copyText = "S.No\tClass Name\tTitle\tDescription\n";
    filteredData.forEach((order, index) => {
      copyText += `${index + 1}\t${order.classname}\t${order.title}\t${order.description}\n`;
    });
                ...
                <TableRow key={order.id}>
                  ...
                  <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                    {Array.isArray(order.files) &&
                      order.files.map((file, i) => (
                        <div key={i}>
                          <a
                            href={`http://localhost:5000/uploads/syllabus/${file}`}
                            target="_blank"
                  ...
                  <TableCell className="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400">
                    <div className="flex gap-3 items-center">
                      <button onClick={() => navigate(`/editsyllabus/${order.id}`)} title="Edit">
