...
export default function BasicTableLeaveCategory() {
    ...
    const fetchleaveCategories = async () => {
      try {
        ...
        const formattedData: Order[] = data.map((leavecategories: any) => ({
          ...
          category: leavecategories.category,
        }));
  ...
  const exportExcel = () => {
    ...
    XLSX.writeFile(workbook, "leave category.xlsx");
  };

  const exportPDF = () => {
  const doc = new jsPDF();
  const tableColumn = ["S.No", "Category","Action"];
  ...
  doc.autoTable({
    ...
    const copyToClipboard = () => {
        let copyText = "S.No\tcategory\n";
        filteredData.forEach((order, index) => {
            copyText += `${index + 1}\t${order.category}\n`;
        });
                ...
                <TableRow key={order.id}>
                  ...
                  <TableCell className="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400">
                    <div className="flex gap-3 items-center">
                      <button onClick={() => navigate(`/editleavecategory/${order.id}`)} title="Edit">
