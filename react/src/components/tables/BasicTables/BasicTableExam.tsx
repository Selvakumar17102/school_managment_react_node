...
export default function BasicTableExam() {
  ...
  const filteredData = useMemo(() => {
    ...
    return tableData.filter((order) => {
      return (
        order.examName.toLowerCase().includes(lowerQuery) ||
  ...
  const exportPDF = () => {
  const doc = new jsPDF();
  const tableColumn = ["S.No", "Exam", "date", "Notes", "Action"];
  ...
  const copyToClipboard = () => {
    let copyText = "S.No\exam\tdate\tNotes\tAction\n";
    filteredData.forEach((order, index) => {
      copyText += `${index + 1}\t${order.examName}\t${order.date}\t${order.notes}\n`;
    });
                ...
                <TableRow key={order.id}>
                  ...
                  <TableCell className="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400">
                    <div className="flex gap-3 items-center">
                      <button onClick={() => navigate(`/editexam/${order.id}`)} title="Edit">
